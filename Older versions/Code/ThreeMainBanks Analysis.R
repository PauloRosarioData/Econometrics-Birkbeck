...
Chart_data <- cumulative_profit
Chart_data$Long <- cumulative_long
Chart_data$Short <- cumulative_short
colnames(Chart_data)[1] <- 'ModelProfit' 
colnames(Chart_data)[2] <- 'LongOnly' 
colnames(Chart_data)[3] <- 'ShortOnly' 
...
a <- VARselect(VAR_dataset, lag.max = 8, type = "both")
p1ct <- VAR(VAR_dataset, p = 4, type = "both")

stargazer(a, title="Results", align=TRUE)

p1ct.irf <- irf(p1ct, response = "PT_var", n.ahead = 28, cumulative = TRUE, boot = TRUE, runs = 1000)
plot(p1ct.irf)
