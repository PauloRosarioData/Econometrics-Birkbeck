...
#Selecting data for the VAR model
myvars <- c("PT_var","BPI_ex_return","BES_ex_return", "BCP_ex_return", "BANIF_ex_return")
dataset = Banks_quotes[myvars]

plot(dataset, nc = 2, xlab = "")

#ADF test
adf1 <- summary(ur.df(dataset[, "PT_var"], type = "trend", lags = 2))


#testing optimal lenght of the VAR
VARselect(dataset, lag.max = 8, type = "both")

#Var model
VAR_MODEL <- VAR(dataset, p = 3, type = "both")

#Impulse Responses
svec.irf <- irf(VAR_MODEL, response = "PT_var", n.ahead = 48, boot = TRUE)
plot(svec.irf)


BCP.irf <- irf(VAR_MODEL, response = "BCP_ex_return", n.ahead = 16, boot = TRUE)
plot(BCP.irf)
